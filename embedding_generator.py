#!/usr/bin/env python3
"""
Vector Embedding Generator for PDF Chunks

This script converts PDF chunks into vector embeddings using sentence transformers.
Supports multiple embedding models and saves embeddings in various formats.
"""
...
class EmbeddingGenerator:
    """Class to generate and manage vector embeddings for PDF chunks."""
    ...
    def generate_embeddings(self, batch_size: int = 32) -> np.ndarray:
        ...
        for i in range(0, len(processed_chunks), batch_size):
            ...
            print(f"Processed batch {i//batch_size + 1}/{(len(processed_chunks)-1)//batch_size + 1}")
    ...
    def save_embeddings(self, output_path: str, format: str = 'numpy'):
        ...
        if format.lower() == 'numpy':
            ...
            data = {
                ...
                'metadata': {
                    ...
                    'model_info': self.available_models.get(self.model_name, 'Unknown')
                }
            }
            ...
            print(f"Embeddings saved as CSV: {output_path}")
...
def main():
    ...
    parser.add_argument('--format', '-f', choices=['numpy', 'pickle', 'json', 'csv'], 
